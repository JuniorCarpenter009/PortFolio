...
export function SiteHeader() {
  ...
  return <header className="site-header"><div className="shell header-inner">
    ...
    <div className="header-actions"><AppearanceToggle /><ThemeToggle /><button className="menu-button" type="button" onClick={() => setOpen(!open)} aria-label="Toggle navigation" aria-expanded={open}>{open ? <X size={20} /> : <Menu size={20} />}</button></div>
  </div></header>;
}
