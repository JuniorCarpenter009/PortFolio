...
import { certifications, education, experience, expertise, products, profile, skillGroups, socialLinks } from "@/data/portfolio";
...
export default function Home() {
  return <><SpaceCanvas /><SiteHeader /><main id="top">
    <section className="hero shell"><div className="hero-copy">
      ...
      <div className="hero-actions"><a className="button primary" href="#work">View selected work <ArrowDownRight size={17} /></a><a className="button secondary" href="/documents/junior-rafael-carpenter-cv.pdf" download>Download CV <ArrowDownRight size={16} /></a><Link className="button text-button" href="/contact">Contact <Mail size={16} /></Link></div>
    ...
    <section id="work" className="section shell"><div className="section-heading"><div><p className="section-number">02 / Selected work</p><h2>Products in motion.</h2></div><p>Built thoughtfully, documented honestly, and presented according to their real stage.</p></div><div className="product-grid">{products.map((product, index) => <article className={`product-card ${product.featured ? "featured" : ""}`} key={product.name}><div className="product-top"><span>0{index + 1}</span><span className="status">{product.status}</span></div><div className="product-icon">{index === 0 ? <Sparkles /> : index === 1 ? <Layers3 /> : <Code2 />}</div><h3>{product.name}</h3><p>{product.description}</p><div className="tags">{product.stack.map((tag) => <span key={tag}>{tag}</span>)}</div>{product.href && <Link className="project-link" href={product.href}>View case study <ArrowUpRight size={14} /></Link>}</article>)}</div>
      <div className="professional-projects"><div className="project-intro"><p className="micro-label">Professional case work</p><h3>Verified outcomes from real delivery.</h3><p>Only public-safe context and metrics already confirmed in my CV are shown here.</p></div>{professionalProjects.map((project) => <article key={project.title}><div className="outcome"><strong>{project.outcome}</strong><span>{project.outcomeLabel}</span></div><div><h3>{project.title}</h3><p>{project.description}</p><small>{project.disclosure}</small></div></article>)}</div>
    </section>
    ...
    <section id="certifications" className="section shell"><div className="section-heading"><div><p className="section-number">06 / Certifications</p><h2>Continuous learning, applied deliberately.</h2></div><p>Selected certifications and courses confirmed in my professional profile. Issuers, dates, and credential links will be added only after individual verification.</p></div><div className="certification-grid">{certifications.map((item, index) => <article key={item.title}><span>0{index + 1}</span><p>{item.category}</p><h3>{item.title}</h3></article>)}</div></section>

    <section id="contact" className="contact-section shell"><p className="section-number">07 / Contact</p><h2>Have a meaningful problem to solve?</h2><Link href="/contact">Start a conversation <ArrowUpRight /></Link></section>
